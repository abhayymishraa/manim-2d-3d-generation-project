...
def run_manim(code: str, job_id: str, output_path: str, quality: str = "m") -> tuple[bool, str]:
    """
    ...
    try:
        ...
        mp4_files = glob.glob(os.path.join(job_output_dir, "**", "Scene.mp4"), recursive=True)
    ...
    try:
        ...
        if success:
            ...
            try:
                ...
                logger.warning(f"Failed to clean up local files for job {job_id}: {str(e)}")
        ...
        try:
            ...
            logger.error(f"Failed to update Supabase for job {job_id}: {str(supabase_err)}")
